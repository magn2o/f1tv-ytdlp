...
const getSessionChannelList = (url) => {
    getContentInfo(url).then((result) => {
        if (isRace(result)) {
            for (const stream of result.metadata.additionalStreams) {
                const data =
                    stream.type === "obc"
                        ? `name: ${config.makeItGreen(stream.driverFirstName + " " + stream.driverLastName)}`.padEnd(
                              37,
                          ) +
                          `number: ${config.makeItGreen(stream.racingNumber)}`.padEnd(
                              22,
                          ) +
                          `tla: ${config.makeItGreen(stream.title)}`
...
const getTokenizedUrl = async (url, content, channel) => {
    ...
    if (content.metadata.additionalStreams == null) {
        ...
        if (isRace(content) && channel == null)
            channel =
                content.metadata.season > 2021 ? "F1 LIVE" : "INTERNATIONAL";
        let stream = getAdditionalStreamsInfo(
            content.metadata.additionalStreams,
            channel,
        );
        let channelId =
            stream.playbackUrl !== null &&
            stream.playbackUrl.indexOf("channelId") == -1
                ? null
                : stream.channelId;
    ...
    try {
                ...
                yarg.positional("url", {
                    ...
                    .option("itsoffset", {
                        ...
                        coerce: (key) => {
                            ...
                            if (!pattern.test(key))
                                throw new Error(
                                    `Invalid format for itsoffset: ${key}. Use (-)hh:mm:ss.SSS`,
                                );
                    ...
                    .option("concurrent-fragments", {
                        type: "number",
                        desc: "Number of fragments of a dash/hlsnative video that should be downloaded concurrently",
                        alias: "N",
                        default: 1,
                    })
                    .option("series-notation", {
                        type: "string",
                        desc: "Fallback series notation for file naming in the event TVDB does not find a match (i.e. S2024E01)",
                        alias: "S",
                    })
                    .option("log-level", {
                        alias: "l",
        ...
        try {
            ...
            if (e.response.status >= 400 && e.response.status <= 499) {
                ...
                if (token == null) {
                    log.info(
                        "You need a valid F1TV login to use this applicaiton.",
                    );
                    log.info(
                        "When are logged into the website, you can use the JavaScript debug console in your browser to retrieve the token using the following JavaScript:\n",
                    );
                    log.info(
                        config.makeItGreen(
                            `\tconsole.log(document.cookie.split(';').filter(cookie => cookie.indexOf('entitlement_token')!=-1)[0].split('=')[1])\n`,
                        ),
                    );
                    const userPrompt = await inquirer.prompt([
                        {
                    ...
                    if (
                        userPrompt.token == null ||
                        userPrompt.token.length == 0
                    )
                ...
                log.info(
                    "Authorization token encrypted and stored for future use at:",
                    config.makeItGreen(
                        `${config.HOME}${config.PATH_SEP}${config.DS_FILENAME}`,
                    ),
                );
                f1tvUrl = await getTokenizedUrl(url, content, channel);
            } else {
        ...
        const seriesNumber =
            seriesNotation != null
                ...
                : await tvdb(
                      ...
                      content.metadata.emfAttributes.sessionEndDate != undefined
                          ? content.metadata.emfAttributes.sessionEndDate
                          : content.properties[0].sessionEndTime,
                      `${content.metadata.title}`,
                  );
        const nameSpec =
            seriesNumber != null
                ? `${series} - ${seriesNumber} - ${content.metadata.title}`
                : `${series} - ${content.metadata.title}`;
        const outFile =
            isRace(content) && channel !== null
                ? `${nameSpec}-${channel.split(" ").shift()}.${ext}`
                : `${nameSpec}.${ext}`;
        const outFileSpec =
            outputDir !== null
                ? outputDir + outFile.replace(/:/g, "-")
                : outFile.replace(/:/g, "-");
        ...
        if (format == "mp4") {
            mp4Metadata.push(
                ...[
                    ...
                    `-metadata`,
                    `season_number=${content.metadata.year}`,
                    `-metadata`,
        ...
        const plDetails = await getProgramStreamId(
            f1tvUrl.url,
            audioStream,
            videoSize,
        );
        ...
        if (useDash && plDetails.hasDRM) {
            log.info(
                `Found DRM Stream ${config.makeItGreen(f1tvUrl.streamType)}.`,
            );
            ...
            try {
                ...
                const ans = await inquirer.prompt([
                    {
                        ...
                        name: "key",
                        message:
                            "Please enter a valid decryption key to download: ",
                        validate: validKey,
                    },
                ]);
                ...
                if (
                    ans.key.split(":")[0].toLowerCase() !=
                    plDetails.kid.toLowerCase()
                )
                    log.info(
                        `KID of the entered key does not match the content. Decryption may not work.`,
                    );
            }
        }
        ...
        const videoSelectString = util.format(
            videoSelectFormatString,
            programStream,
        );
        const audioSelectString = useDash ? "0:a" : `0:p:${programStream}:a`;

        log.debug(
            `Video selection: ${videoSelectString} / Audio selection: ${audioSelectString}`,
        );
        ...
        if (audioStreamId !== -1) {
            log.info(
                `Found audio stream that matches ${config.makeItGreen(audioStream)}.`,
            );
            log.info(
                `Using audio stream id ${config.makeItGreen(audioStreamId)}.`,
            );
        } else {
            log.info(
                `Unable to find a match for audio stream that matches ${config.makeItGreen(audioStream)}.`,
            );
        ...
        if (includeInternationalAudio && isRace(content)) {
            log.info(
                `Adding ${internationalAudio} commentary from the international feed as a second audio channel.`,
            );
            log.debug(itsoffset);

            intlUrl = await getTokenizedUrl(url, content, "INTERNATIONAL");
            const intlDetails = await getProgramStreamId(
                intlUrl.url,
                internationalAudio,
                "480x270",
            );
            ...
            if (intlDetails.hasDRM && intlDetails.key != undefined) {
                log.info(
                    `Adding download of additional audio (${internationalAudio}) encrypted stream.`,
                );
                drmStreams.addStream(
                    new AudioStream(
                        intlUrl.url,
                        `${nameSpec}-audio-${internationalAudio}.m4a`,
                        intlDetails.key,
                        internationalAudio,
                    ),
                );
            ...
            const intlAudioSelectFormatString = useDash
                ? "1:a:m:id:%i"
                : `1:p:${programStream}:a:%i`;
            const intlAudioSelectString = util.format(
                intlAudioSelectFormatString,
                intlDetails.audioId,
            );
            ...
            audioStreamMapping = [
                ...
                audioSelectString,
                "-map",
                intlAudioSelectString,
            ];

            let intlLangId =
                internationalAudio == "eng" ? "Sky" : internationalAudio;
        ...
        if (plDetails.hasDRM && plDetails.key != undefined) {
            ...
            for (const stream of drmStreams) {
                ...
                download.on("data", (data) => {
                    ...
                    process.stdout.write(
                        `\r${drmStreamProgressTracker.toString()}`,
                    );
                });
            ...
            for (const stream of drmStreams) {
                promises.push(
                    bento4.mp4decrypt.exec(stream.decFilename, [
                        "--key",
                        stream.key,
                        stream.encFilename,
                    ]),
            ...
            const streamMapping =
                includeInternationalAudio && isRace(content)
            ...
            return includeInternationalAudio && isRace(content)
                ? ffmpeg()
                      ...
                      .inputOptions(["-itsoffset", itsoffset])
                      .outputOptions(options)
                      .on("start", (commandLine) => {
                          log.info(
                              "Muxing decrypted streams:",
                              config.makeItGreen(commandLine),
                          );
                          log.debug(
                              "Executing command:",
                              config.makeItGreen(commandLine),
                      ...
                      .on("progress", (info) => {
                          const outStr =
                              "\rFrames=" +
                              config.makeItGreen(
                                  fixedLength(`${info.frames}`, 10),
                              ) +
                              " Fps=" +
                              config.makeItGreen(
                                  fixedLength(`${info.currentFps}`, 5) + "fps",
                              ) +
                              " Kbps=" +
                              config.makeItGreen(
                                  fixedLength(`${info.currentKbps}`, 7) +
                                      "Kbps",
                              ) +
                              " Duration=" +
                              config.makeItGreen(
                                  fixedLength(`${info.timemark}`, 11),
                              ) +
                              " Percent Complete=" +
                              config.makeItGreen(
                                  fixedLength(`${parseInt(info.percent)}`, 3) +
                                      "%",
                              );
            ...
            ? // Use this command when adding international audio
              ffmpeg()
                  ...
                  .on("codecData", (data) => {
                      ...
                      log.info(
                          "File duration:",
                          config.makeItGreen(data.duration),
                          "\n",
                      );
                  })
                  .on("progress", (info) => {
                      const outStr =
                          ...
                          " Kbps=" +
                          config.makeItGreen(
                              fixedLength(`${info.currentKbps}`, 7) + "Kbps",
                          ) +
                          " Duration=" +
                          config.makeItGreen(
                              fixedLength(`${info.timemark}`, 11),
            ...
            : // Use this command for everything else
              new ytdlpwrap()
                  ...
                  .on("progress", (info) => {
                      const outStr =
                          "\rETA=" +
                          config.makeItGreen(fixedLength(`${info.eta}`, 6)) +
                          " Speed=" +
                          config.makeItGreen(
                              fixedLength(`${info.currentSpeed}`, 12),
                          ) +
                          " Size=" +
                          config.makeItGreen(
                              fixedLength(`${info.totalSize}`, 10),
                          ) +
                          " Percent Complete=" +
                          config.makeItGreen(
                              fixedLength(`${parseInt(info.percent)}`, 3) + "%",
                  ...
                  .on("error", (e) => {
                      log.error("ytdlp error:", e.message);
                      log.info(e);
                  });
    } catch (e) {
